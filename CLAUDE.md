...
- **Tipografia:** `Fraunces` (display/títulos), `DM Sans` (corpo), `IBM Plex Mono` (dados: números, datas, status, labels — dá uma linguagem de "manifesto/ficha", separa prosa de dado), `Cinzel` (SÓ o wordmark — capitulares romanas, dão o ar "classe alta" que o Lucas pediu). Playfair Display foi removido de vez.
- **Logo:** wordmark "SPOT" em Cinzel maiúsculo com `letter-spacing` 6, e uma **esfera armilar** (círculo + anel elíptico inclinado -22° passando por fora) no lugar do "O", em terracota `#c1552f`. Mesma marca no favicon, no manifest e no PNG 180×180 do ícone iOS.
  - Geometria travada por medição de tinta, não a olho: marca com 67.8 de altura contra 66.3 de altura de maiúscula (overshoot de 2.3%, o mesmo de uma letra redonda numa serifada), centro em x=159.3 — que **não** é o meio do avanço do "O", porque o `letter-spacing` entra no avanço. Se mexer na fonte ou no espaçamento, remedir.
  - Histórico de rejeição (não reabrir sem pedido): pin de localização → anel tracejado + ponto → círculo com elipse sozinha ("parece um olho") → círculo com elipse + equador ("bola de basquete" — simetria + laranja). O que resolveu foi quebrar a simetria com o anel inclinado.
- **Nome:** continua **Spot**. Foram avaliados Sojourn, Meridian, Dossier e Lodestar com checagem de domínio; Lucas preferiu manter o nome e trocar só a identidade. Cores também ficaram como estavam — ele pediu explicitamente pra não mexer.
